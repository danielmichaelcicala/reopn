...
\Daniel{
  \begin{itemize}
  \item systematic use of emph, `' and `` `` and
    bold face
  \item RGRAPH vs GRAPH? To evangelize the
    poisitoin of RGRAPHs superiority in CT over
    GRPH or not?
  \end{itemize}
  }
...
\input{assets/intro_webpage03.tex} gives the graph
inputs `page b' and `page c' and outputs
`home'. Likewise
\input{assets/intro_webpage04.tex} gives the graph
...
\input{assets/intro_webpage05.tex} This example
...
\begin{definition} \label{df:str_csp_category}
  ...
  $ L' \dashv R' \from \X' \to \A' $ be geometric
  morphisms. A \df{morphsim of structured cospan categories}
...
contains the rule \input{assets/decompSquare.tex} for each
...
\begin{theorem} \label{thm:inductive-rewriting}
  ...
  \input{assets/inductiveRewriting1.tex} in the
  double category
...
\begin{proof}
  ...
  \input{assets/inductiveRewriting2.tex} then the
...
\input{assets/inductiveRewriting10.tex}

Rewriting can be performed on each of these sub-systems
\input{assets/inductiveRewriting11.tex} The composite of
