% 
% This is the source file for the paper ``rewriting open objects''. This
% version uses categories A and X that are not necessarily topoi. 
% 
...
\section{Open objects} \label{sec:open-objects}
...
\label{thm:dpo_category-StrCsp-adhsv}
  ...
  The category $ \Cospan_L $ is adhesive.
...
\begin{lemma}
  ...
  Consider a cocartesian categories $ (\A , + , 0_{\A} , \tau_\A ) $
  and $ (\X , \otimes_{\X} , I_{\X}) $ such that \( \A \) has
  pullbacks, \( \X \) has pushouts and both are locally
  cartesian closed.
...
\begin{exposition}
  ...
  \]
  This rule rewrites \( La \to x \gets Lb \)
  into \( Le \to z \gets Lf \).  Take an arrow
  \[
    ( x from La \tocospan Lb ) \to (w \from La' \tocospan Lb')
  \]
  of structured cospans such that the composite formed with
  \[
    ( y \from Lc \tocospan Ld) \to (x \from La \tocospan Lb)
  \]
...
\begin{lemma}
  ...
  Consider a cocartesian categories $ (\A , + , 0_{\A} , \tau_\A ) $
  and $ (\X , \otimes_{\X} , I_{\X}) $ such that \( \A \) is locally
  cartesian closed and \( \X \) is a topos.  Fix an adjunction
  $ L \dashv R \from \A \to \X $ with $ L $ preserving pullbacks.
